...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------
    p2 = posj(25, 0, 0, 0, 0, 0)  #given position in relative mode
    print("Given position: ",p2)
    clientsocket, address = s.accept()
    clientsocket.send(bytes(str(p2), "utf-8"))
    ...
    if (pos1[5] < 0):
        list2[5] = -(list2[5])
    print("max limit from current pos:  ",list2)
    
    pos = [] 
    if pos1[0] == 0:
        item = (pos1[0] % list2[0])#calculate max limit from home condition
        pos.append(item)
    else:                       
        item = list2[0] % pos1[0]
        pos.append(item)
    ...
    if pos1[5] == 0:
        ...
        item = list2[5] % pos1[5]
        pos.append(item)
    print("pos to cover max limit from current pos:",pos) 
    ...
    while True:
        if a != b:
            ...
            movej(n_list, vel=60, acc=30, mod = DR_MV_MOD_REL)
            param = get_last_alarm().param
            clientsocket, address = s.accept()
