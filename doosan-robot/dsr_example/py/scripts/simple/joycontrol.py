...
def callback(msg_mqtt_sub):
     
    
    joy_x=round(100*msg_mqtt_sub.message[0],1)
    joy_y=round(100*msg_mqtt_sub.message[1],1)
    joy_z=round(100*msg_mqtt_sub.message[2],1)
    joy_rx=round(100*msg_mqtt_sub.message[3],1)
    joy_ry=round(100*msg_mqtt_sub.message[4],1)
    joy_rz=round(100*msg_mqtt_sub.message[5],1)
    # robo_ready_pos=d.buttons[0]
    print(msg_mqtt_sub.message[0])
    print(msg_mqtt_sub.message[1])
    print(msg_mqtt_sub.message[2])
    print(msg_mqtt_sub.message[3])
    print(msg_mqtt_sub.message[4])
    print(msg_mqtt_sub.message[5])
...
def start():
        rospy.Subscriber("/mqtt_sub", msgMqttSub,callback)
