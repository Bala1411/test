...
# def msgOnLogAlarm_cb(msg):
  
#     pub_error= rospy.Publisher('/error', RobotError, queue_size=10)

#     msgOnLogAlarm_cb.count += 1

#     if (0==(msgOnLogAlarm_cb.count % 100)): 
#         rospy.loginfo("________ ROBOT LOG ________")
#         print("  level                 : %d" % (msg.level))
#         print("  group                 : %d" % (msg.group))
#         print("  index                 : %d" % (msg.index))
#         print("  param                 : %s" % (msg.param[0]))
#         print("  param                 : %s" % (msg.param[1]))
#         print("  param                 : %s" % (msg.param[2]))
#         pub_error.publish(msg)        

# msgOnLogAlarm_cb.count = 0       
...
def thread_subscriber():
    ...
    rospy.spin()
#     #rospy.spinner(2)  
# def thread_subscriber2():
#      rospy.Subscriber('/error', RobotError, queue_size=100)
#      rospy.spin()
# # def thread3():
#     rospy.Subscriber('/'+ROBOT_ID +ROBOT_MODEL+'/rosout', Log, msgRobotState_cb)
#     rospy.spin()    

if __name__ == "__main__":
    rospy.init_node('single_robot_simple_py_a0509')
    rospy.on_shutdown(shutdown)
    set_robot_mode  = rospy.ServiceProxy('/'+ROBOT_ID +ROBOT_MODEL+'/system/set_robot_mode', SetRobotMode)
    Thread(target = thread_subscriber).start()
    #Thread(target = thread_subscriber2).start()
    ...
    #rospy.Subscriber('/'+ROBOT_ID +ROBOT_MODEL+'/error', RobotError, msgOnLogAlarm_cb)       
    ...
    p2= posj(10.0, 0.0, 90.0, 10.0, 90.0, 10.0) #joint

    x1= posx(200, 37.6, 540.5, 45, 180, 45) #task
    x2= posx(750, 340.5, 350.5, 95, 155, 75) #task
